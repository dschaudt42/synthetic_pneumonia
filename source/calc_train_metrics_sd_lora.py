...
for label,train_folder in tqdm(zip(labels,train_folders)):
    ...
    for folder in glob.glob(f'images/sd_lora/1e-5/sd_lora_scale1/{label}/*'):
    ...
    df.to_csv(f'export/train_metrics/sd_lora/1e-5/sd_lora_scale1/{label}_train_metrics.csv',index=False)
...
for label,train_folder in tqdm(zip(labels,train_folders)):
    ...
    for folder in glob.glob(f'images/sd_lora/1e-5/sd_lora_scale05/{label}/*'):
        iteration_num = Path(folder).name[11:]
        diffusion_image_files = glob.glob(f'{folder}/*')
    ...
    df = create_df(iteration_list,fid_list,ms_ssim_list,train_image_folder)
    df.to_csv(f'export/train_metrics/sd_lora/1e-5/sd_lora_scale05/{label}_train_metrics.csv',index=False)
